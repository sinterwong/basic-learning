...
xdb_bench_t::xdb_bench_t(const std::string &file_name)
    : xdb_search(file_name) {}

void xdb_bench_t::init_file() { xdb_search.init_file(); }

void xdb_bench_t::init_vector_index() { xdb_search.init_vector_index(); }

void xdb_bench_t::init_content() { xdb_search.init_content(); }
...
void xdb_bench_t::bench(const std::string &file_name) {
  ...
  printf("total: %llu, took: %.2f s, cost: %.2f μs/op, io "
         "count: "
         "%llu\n",
         sum_count, took, cost, sum_io_count);
}
